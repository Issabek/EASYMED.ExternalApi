...
namespace RealtyPortal.ExternalApi.Controllers
{
    ...
    public class QRSignController : ControllerBase
    {
        ...
        private string ConnectionString
        {
            get
            {
                if (_config.GetValue<bool>("IsTest"))
                    return _config.GetValue<string>("ConnectionStrings:SessionAuthConnectionString");
                else
                    return Crypt.Decrypt(_config.GetValue<string>("ConnectionStrings:SessionAuthConnectionString"), "DCStrPN10012018", 0);
            }
        }
        private string ApiPassword => _config.GetValue<string>("ApplicationSettings:ApiPassword");
        private bool IsTest => _config.GetValue<bool>("IsTest");
        private string TokenAuthUrl => _config.GetValue<string>("CBSProxy_TokenAuthWSSoap");
        private string ClientUrl => _config.GetValue<string>("CBSProxy_ClientWS");
        private string SystemId => Crypt.Decrypt(_config.GetValue<string>("CBSPROXY_SystemID"), "DdgtemId7", 0);
        private string SystemLogin => Crypt.Decrypt(_config.GetValue<string>("CBSPROXY_Login"), "trohgFgh238476", 0);
        private string SystemPassword => Crypt.Decrypt(_config.GetValue<string>("CBSPROXY_Password"), "fdgwoDd6784g6d4fg", 0);
        ...
        [HttpGet]
        public Dictionary<Guid, string> GetStatusList()
        {
            try
            {
                Dictionary<Guid, string> statusDict = new Dictionary<Guid, string>();
                using (IDbConnection db = new SqlConnection(ConnectionString))
                {
                    foreach (PnStatus stat in db.Query<PnStatus>("select * from pn_status where type=5"))
                    {
                        statusDict.Add(stat.id, stat.code);
                    }
                }
                return statusDict;
            }
            catch (Exception ex)
            {
                ...
                dict.Add(Guid.NewGuid(), ex.Message);
